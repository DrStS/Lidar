...
void clusterRecursive(int index, const std::vector<std::vector<float>> points, std::vector<int> &cluster, std::vector<bool> &processed, KdTree *tree, float distanceTol)
{
	processed[index] = true;
	cluster.push_back(index);
	std::vector<int> nnPointsIndex = tree->search(points[index], distanceTol);

	for (int id : nnPointsIndex)
	{
		if (!processed[id])
			clusterRecursive(id, points, cluster, processed, tree, distanceTol);
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol)
{
	...
	for (int i; i < points.size(); i++)
	{
		if (!processed[i])
		{
			...
			clusterRecursive(i, points, cluster, processed, tree, distanceTol);
